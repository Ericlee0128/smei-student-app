...
# Custom CSS for better styling - IMPROVED for better UX
st.markdown("""
<style>
    .main-header {
        font-size: 2.5rem;
...
st.title("🎓 SMEI Student Progression Tracker")

# Load data
df = load_student_data()

# Quick Stats in Sidebar - IMPROVED: Focus on urgent issues
st.sidebar.header("📊 Quick Overview")
...
if not df.empty:
    ...
    finishing_soon = len(df[(df['Finish Date'] >= pd.Timestamp.now()) & 
                           (df['Finish Date'] <= pd.Timestamp.now() + pd.Timedelta(days=30))])
    ...
    st.sidebar.markdown("---")
    st.sidebar.header("🚨 Urgent Attention Needed")
    st.sidebar.metric("Low Attendance (<80%)", low_attendance)
    st.sidebar.metric("Poor Progression (<50%)", low_progression)
    st.sidebar.metric("Finishing Soon (30 days)", finishing_soon)

# Main Dashboard - NEW: High-level overview
if not df.empty:
...
# Search and Filter Section - IMPROVED: Simplified and more intuitive
st.markdown('<div class="filter-section">', unsafe_allow_html=True)
st.subheader("🔍 Find Students")

# Main navigation - SIMPLIFIED: Only two clear options
nav_option = st.radio(
    ...
    if nav_option == "Find Specific Student":
        ...
        if assessment_search != "Select an assessment":
            ...
            if not assessment_results.empty:
                ...
                if st.button("📋 Export Contact List for Follow-up"):
...
st.markdown(
    ...
    <div style='text-align: center; color: #666;'>
        📧 SMEI Student Progression Tracker | For support contact administration
    </div>
    """,
    unsafe_allow_html=True
)
